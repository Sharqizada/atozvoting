...
const handleScannedCode = async (scannedValue, format) => {
...
const openCamera = async () => {
  ...
  try {
    ...
    scannerMessage.value = 'Camera is ready. Hold the barcode or QR code close and steady.'
  } catch (error) {
...
const verifyBadge = async (fromScanner = false) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/auth/badge`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      detectedAdmin.value = null
      errorMessage.value = data.message || 'Badge verification failed.'
      return false
    }
...
const submitLogin = async () => {
  ...
  try {
    ...
    if (rememberMe.value) {
      ...
      sessionStorage.setItem('adminSession', JSON.stringify(data.admin))
    }
    ...
    <main class="flex min-h-screen items-center justify-center px-4 py-6 sm:px-6">
      <div class="w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-5 shadow-xl sm:max-w-lg sm:p-7">
        <div class="flex items-center justify-center gap-3">
          <div
            class="flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 text-blue-600 sm:h-16 sm:w-16"
          >
        ...
        <div class="mt-6 space-y-4">
          <div class="rounded-3xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
            ...
            <button
              type="button"
              @click="router.push('/')"
              class="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-4 text-base font-medium text-slate-700 transition hover:bg-slate-100"
            >
        ...
        <div class="mt-4 rounded-3xl border border-dashed border-blue-200 bg-slate-50 p-3">
          <div class="relative overflow-hidden rounded-[24px] border-2 border-dashed border-blue-200 bg-slate-950">
            ...
            <div class="pointer-events-none absolute inset-0">
              <div class="absolute left-4 top-4 h-8 w-8 border-l-4 border-t-4 border-blue-500"></div>
              <div class="absolute right-4 top-4 h-8 w-8 border-r-4 border-t-4 border-blue-500"></div>
              <div class="absolute bottom-4 left-4 h-8 w-8 border-b-4 border-l-4 border-blue-500"></div>
              <div class="absolute bottom-4 right-4 h-8 w-8 border-b-4 border-r-4 border-blue-500"></div>
              <div
